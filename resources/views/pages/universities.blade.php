  ...
  <div class="container">
    ...
    <nav class="breadcrumb-wrapper">
      <ol class="breadcrumb">
        ...
        <li class="<?php if (count($breadcrumbs)==0){ echo 'active'; } ?>">
            ...
            Университеты
            @else
            <a href="{{url('/universitety')}}">Университеты</a>
            @endif
        </li>
    ...
    <div class="hero">
      <div class="hero-text has-size">
        <h1 class="_change_able" data-key="universities_info_h1{{$city}}{{$direction}}" data-page="{{($_GET['_page']??'').$city.$direction}}">{{$template['universities_info_h1'.$city.$direction]??__('global_empty')}}</h1>
        <p class="_change_able" data-key="universities_info_p{{$city}}{{$direction}}" data-page="{{($_GET['_page']??'').$city.$direction}}">{{$template['universities_info_p'.$city.$direction]??__('global_empty')}}</p>        
      </div>
    ...
    <!-- honest -->
    <div class="honest">
        <h2 class="_change_able" data-key="universities_honest_h2{{$city}}{{$direction}}" data-page="{{($_GET['_page']??'').$city.$direction}}">{{$template['universities_honest_h2'.$city.$direction]??__('global_empty')}}</h2>
        <p class="_change_able" data-key="universities_honest_p{{$city}}{{$direction}}" data-page="{{($_GET['_page']??'').$city.$direction}}">{{$template['universities_honest_p'.$city.$direction]??__('global_empty')}}</p>
...
<div class="container">
    ...
    <!-- text -->
    <div class="text">
        <h3 class="_change_able" data-key="universities_last_info_h3{{$city}}{{$direction}}" data-page="{{($_GET['_page']??'').$city.$direction}}">{{$template['universities_last_info_h3'.$city.$direction]??__('global_empty')}}</h3>
        <p class="_change_able" data-key="universities_last_info_p{{$city}}{{$direction}}" data-page="{{($_GET['_page']??'').$city.$direction}}">
            <?php 
                if (strlen($template['universities_last_info_p'.$city.$direction]??'')>500){ 
                    echo substr($template['universities_last_info_p'.$city.$direction], 0, 500).'<span class="big-dots">...</span><span class="extra-text d-none">'.substr($template['universities_last_info_p'.$city.$direction], 500).'<span>';
                }else{
                    echo $template['universities_last_info_p'.$city.$direction]??__('global_empty');
                } 
            ?>
        </p>
        <?php if (strlen($template['universities_last_info_p'.$city.$direction]??'')>500){ ?>
